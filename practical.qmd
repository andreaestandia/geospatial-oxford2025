...
A working R installation (R >= 4.0) and the packages listed below. You only need to install packages once; subsequent runs will load the cached versions.
...
# Create folders
folders <- c("data/raw", "data/processed", "outputs/maps", "outputs/models")
for (f in folders) {
...
Create example sampling sites and convert to an `sf` object.
...
cat("Distance in WGS84 (metres):", round(dist_wgs84), "\n")
...
cat("Distance in BNG (metres):", round(dist_bng), "\n")
...
cat("Distance in Web Mercator (metres):", round(dist_merc), "\n")

# Show coordinate values in different systems
cat("\nOxford coordinates:\n")
cat("  WGS84 (lon, lat):", crds(oxford), "\n")
cat("  BNG (easting, northing):", crds(oxford_bng), "\n")
cat("  Web Mercator (x, y):", crds(oxford_merc), "\n")
```
...
head(species_data)
# Save processed data
write_csv(species_data, file.path("data", "processed", "coffea_arabica_processed.csv"))
```
...
This step evaluates how well the model distinguishes between suitable (presence) and unsuitable (background/pseudo-absence) environments. The dataset is split into training (70%) and testing (30%) subsets to assess the model’s predictive performance on unseen data.
...
ggsave(file.path("outputs", "maps", "pretty_map.pdf"), 
       pretty_map, 
