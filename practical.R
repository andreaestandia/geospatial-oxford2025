...
# Nilo Merino Recalde (nilo.recalde@biology.ox.ac.uk)
# Based on tutorials from: https://rspatial.org/raster/sdm/
# ============================================================================

# Introduction ---------------------------------------------------------------
# Welcome to this practical on geospatial analysis in R for 3rd Year MBiol 
# students. Over the next ~2 hours you'll learn core GIS concepts and apply 
# them to a case study: modelling species distributions for Coffea arabica 
# (Arabica coffee).
...
# Reproducibility, easy sharing, and avoiding accidental modification of raw data.
...
# Create example sampling sites and convert to a spatial object
...
# All spatial data must have a coordinate reference system (CRS) which is a 
# framework used to precisely measure locations on the surface of Earth as 
# coordinates. If two layers have different CRSs you will misalign layers when 
# overlaying, measuring distances, or extracting raster values.
...
# Create a small example raster. Rasters represent data that vary continuously 
# across space. Each cell (pixel) stores a numeric value for that location — 
# just like pixels in a photo store color values. Common examples include 
# environmental data, elevation, vegetation indices or satellite images.
...
# Now we're going to plot the three types of object we have generated: 
# a polygon (UK map), points (species counts), and a raster (environmental data).
...
# 
# Important: Because GBIF downloads can be large, we're going to download the 
# file from Canvas or 
# https://github.com/andreaestandia/geospatial-oxford2025.git/data/raw/coffea_arabica.rds 
# and place it in data/raw. If you don't have this file in that folder, it will 
# download it for you, but take into account that this might be slow.
...
# Why remove ocean points? Some occurrences fall in the sea due to reporting 
# errors. We remove points that intersect ocean polygons. First, you will 
# download the ocean polygons and store them in data/raw.
...
# WorldClim is a global climate database providing gridded climate data 
# (temperature, precipitation, etc.) derived from weather stations.
# 
# You're using 19 bioclimatic ("bio") variables that summarise key aspects of 
# climate — such as averages, seasonality, and extremes — that are biologically 
# meaningful and commonly used in ecological and species distribution modelling. 
# You can find out what each variable means here: 
# https://www.worldclim.org/data/bioclim.html
# 
# We will download a moderate-resolution set (10 arc-minute for speed). 
# Files will be stored in data/raw.
...
# We apply a buffer (±5 degrees) around the species' occurrence points to 
# define a broader study area. This ensures that when you crop the climate data, 
# you capture not only where the species is found but also nearby environments — 
# useful for modelling potential distributions and avoiding edge effects.
...
head(species_data)
# Save processed data
write.csv(species_data, here(file.path("data", "processed", "coffea_arabica_processed.csv")))


# Part 5 — Species distribution modelling ------------------------------------

# We use a simple Generalised Linear Model but there are many alternatives 
# (GAM, Random Forest, MaxEnt...).

## 5.1 Create background (pseudo-absence) points

# We will build a model to predict species presence (1) or absence (0) using 
# bioclimatic variables. However, we don't have true absence data—locations 
# where we are certain the species is not present. Without absence data, the 
# model cannot distinguish between areas where the species might be absent and 
# areas we simply haven't surveyed. To address this, we generate "pseudo-absence" 
# data by sampling random background points from the region, assuming these 
# locations represent areas where the species is unlikely to occur. Pseudo-absence 
# is essential because it provides a contrast to presence data, allowing the model 
# to learn patterns associated with species occurrence.
...
# Question: What are the limitations of using random background points? 
# (Think about sampling bias and non-targeted absences.)


## 5.2 Prepare training table and fit a GLM

# Now we are ready to fit a logistic regression to predict presence/absence. 
# We use a general linear model assuming binomial errors, which is appropriate 
# for modelling a binary Y variable of 0s and 1s. More on this in Year 2 lecture 
# Analysis of associations Part III: Non-Linear Regression Bonsall, Michael, 
# and in Year 4!
# 
# We chose bio1 (Annual Mean Temperature), bio12 (Annual Precipitation), and 
# bio7 (Temperature Annual Range) because they capture the important climatic 
# factors that typically influence species distributions: bio1 represents overall 
# thermal conditions, bio12 reflects water availability, and bio7 describes 
# temperature variability or seasonality.
...
# Question: Which variables significantly predict presence? 
# What does it mean biologically?


## 5.3 Evaluate the models (AUC, ROC)

# This step evaluates how well the model distinguishes between suitable (presence) 
# and unsuitable (background/pseudo-absence) environments.
# 
# We use two key metrics:
# 
# - ROC curve (Receiver Operating Characteristic): shows the trade-off between 
#   true positives and false positives across different thresholds.
# - AUC (Area Under the Curve): summarizes the ROC; values range from 0.5 
#   (no better than random) to 1.0 (perfect discrimination).
# 
# A higher AUC means the model reliably distinguishes where the species is likely 
# present versus absent — a crucial check before interpreting or mapping predictions.
...
#   model selection should be guided by theory and research questions, not just 
#   by AIC or p-values
# * Consider multiple evaluation metrics and use cross-validation when possible
# * Think about your aims: do you want to predict well, or understand the 
#   underlying biology?
# * The more variables you test, the more likely you are to overfit the observed 
#   data, which can lead to poor predictions on new data

# Question: Which bioclimatic variables do you expect to be most important for 
# Coffea arabica? Does a model including these variables predict distribution 
...
# NOTE: eval=FALSE in original - uncomment to run
...
# Creating pretty maps is highly subjective — but there are a few tricks that 
# can help. We can adjust fonts, move legends, and change color palettes 
# depending on the type of data we have. The following code demonstrates how to 
# refine the visual appearance of our species suitability map.
...
# We're now cropping the map by computing data extent from pred_df 
# (only where suitability is present)
...
# We're going to use a different font. In this case, we've chosen Roboto 
# Condensed from Google Fonts
...
# Question: Our points are not shown in this map as those lines of code have 
# been muted. If we were to plot them, what colour palette would be best to use 
# for the map and for the point so they can be seen? What about the size of the 
# points?
